...
def train_single(
    ...
    print(f'\n=============== threshold_get_subjects: {threshold_i}, threshold_get_students: {threshold_j} ===============')
    ...
    for index_user, user in tqdm(enumerate(lst_users)):
        ...
        if index_user % 100 == 99:
            print(f'\n{index_user + 1} students, threshold_get_subjects: {threshold_i}, threshold_get_students: {threshold_j}, Score: {score * 100 / (index_user + 1)}')
    ...
    with open(f'./results/result_trainsingle_{save_file}.log', 'a') as file:
        file.write('{')
        file.write(f'threshold_get_subjects: {threshold_i}, threshold_get_students: {threshold_j}, score: {score * 100 / (index_user + 1)}, count: {lst_count}')
        file.write('}\n')
